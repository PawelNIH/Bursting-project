...
ggplot(Pim_filtered, aes(y= (norm_expr), x= (off_time)))  + 
  theme_classic() +
  geom_point(aes(colour = Cell), size = 5, 
  ) + stat_cor(method = "kendall", size = 5) +
  geom_smooth(method='lm', se = FALSE) +
  geom_errorbar(aes(ymin = norm_expr - norm_SEM_expr, ymax = norm_expr + norm_SEM_expr))+ 
  geom_errorbar(aes(xmin = off_time - off_time_MAD, xmax = off_time + off_time_MAD)) + ylim(-1, 11)


ggplot(Pim_filtered, aes(y= (norm_expr), x= (norm_BS)))  + 
  theme_classic() +
  geom_point(aes(colour = Cell),size = 5, 
  ) + stat_cor(method = "kendall", size = 5) +
  geom_smooth(method='lm', se = FALSE)  +
  geom_errorbar(aes(ymin = norm_expr - norm_SEM_expr, ymax = norm_expr + norm_SEM_expr)) + 
  geom_errorbar(aes(xmin = norm_BS - norm_BS_MAD, xmax = norm_BS + norm_BS_MAD)) +  ylim(-1, 11)
