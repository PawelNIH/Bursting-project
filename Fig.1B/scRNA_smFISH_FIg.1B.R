...
sc <- filter(sc, Gene == "MYC" | Gene == "SOX9" | Gene == "ERRFI1" | Gene =="HMMR" | Gene == "KIF2C" | Gene == "PPM1G" | Gene == "PSMD14")
...
FISH <- filter(FISH, Gene == "MYC" | Gene == "SOX9" | Gene == "ERRFI1" | Gene =="HMMR" | Gene == "KIF2C" | Gene == "PPM1G" | Gene == "PSMD14")


sc_FISH <-  bind_cols(sc, FISH)

##
ggplot(sc_FISH, aes(y= log10(Expression...15), x= log10(Expression...30))) +  geom_point(size = 2) +
  geom_smooth(method='lm') +theme_classic() + xlab("log10 smRNA-FISH expression [mean counts]")  + ylab("log10 scRNA-Seq expression [mean counts]")
...
ggplot(sc_FISH, aes(y= log10_Rate01Median...2, x= log10_Rate01Median...17)) +  geom_point(size = 2) +
  stat_cor(method = "pearson") + 
  geom_smooth(method='lm') +theme_classic()+
  stat_ma_line(method = "RMA",
               range.y = "interval",
               range.x = "interval")+ stat_ma_eq(method = "RMA", range.y = "interval",
                                                 range.x = "interval")+ stat_cor(method = "pearson", color = 'red') + 
  geom_smooth(method='lm', color = "red")



ggplot(sc_FISH, aes(y= log10_Rate10Median...5, x= log10_Rate10Median...20)) +  geom_point(size = 2) +
  stat_cor(method = "pearson") + 
  geom_smooth(method='lm') +theme_classic()+
  stat_ma_line(method = "RMA",
               range.y = "interval",
...
ggplot(sc_FISH, aes(y= log10_EjectMedian...8, x= log10_EjectMedian...23)) +  geom_point(size = 2) +
  stat_cor(method = "pearson") + 
  geom_smooth(method='lm') +theme_classic()+
  stat_ma_line(method = "RMA",
               range.y = "interval",
...
ggplot(sc_FISH, aes(y= log10_BurstMedian...11, x= log10_BurstMedian...26)) +  geom_point(size = 2) +
  stat_cor(method = "pearson") + 
  geom_smooth(method='lm') +theme_classic()+
  stat_ma_line(method = "RMA",
               range.y = "interval",
