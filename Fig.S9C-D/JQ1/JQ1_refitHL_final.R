...
all_DMSO <- read.csv(file = 'refit_HL_DMSOJQ_Summary_scRNA-ss-HCT116D_2.csv')

all_burst_DMSO <- read.csv(file = 'refit_HL_DMSOJQ_burst_scRNA-ss-HCT116D_HCTDMSOr2_2.csv')
...
all_JQ1 <- read.csv(file = 'refit_HL_JQ1_Summary_scRNA-ss-HCT116J_2.csv')

all_burst_JQ1 <- read.csv(file = 'refit_HL_JQ1_burst_scRNA-ss-HCT116J_HCTJQ1r2_2.csv')
...
DMSO_JQ1_horizontal_filter <- transform(DMSO_JQ1_horizontal_filter, LFC_eject = log2(EjectMedian.y/EjectMedian.x))
...
down <- read.csv(file = 'jq1_down.csv')
others <- read.csv(file = 'jq1_others.csv')
...
DMSO_JQ1_horizontal_filter$QC <- ifelse(DMSO_JQ1_horizontal_filter$Gene %in% QC$Gene, "yes", "no") 

DMSO_JQ1_horizontal_filter <- filter(DMSO_JQ1_horizontal_filter, QC == "yes")
...
ggplot(DMSO_JQ1_horizontal_filter, aes(x=cat_JQ1_120_bulk, y= log2(Expression.y/Expression.x), color=cat_JQ1_120_bulk))  + 
  ...
  ylab("log2 (JQ1/DMSO) Expression") + ylim(-4, 4) 


a <- ggplot(DMSO_JQ1_horizontal_filter, aes(x=cat_JQ1_120_bulk, y= LFC_off, color=cat_JQ1_120_bulk))  +   geom_boxplot(width = 0.3) + theme_classic2()  + 
  theme(axis.title.x=element_blank(),
        axis.text.x=element_blank(),
        axis.ticks.x=element_blank()) +   
  scale_color_brewer(palette="Set1") +  theme(axis.text=element_text(size=12),
...
b <- ggplot(DMSO_JQ1_horizontal_filter, aes(x=cat_JQ1_120_bulk, y= LFC_BS, color=cat_JQ1_120_bulk))   + 
  geom_boxplot(width = 0.3) + theme_classic2()  + 
  theme(axis.title.x=element_blank(),
        axis.text.x=element_blank(),
        axis.ticks.x=element_blank()) +   
